...
public class WorkoutServiceTests
{
    ...
    [Test]
    public void CreateAddWorkoutSetViewModel_WorkoutContainsExercisesAndProgressions_DoesReturnViewModelWithExactItems()
    {
        ...
        AddWorkoutSetViewModel result = this._service.CreateAddWorkoutSetViewModel(workout);
        ...
        Assert.That(result.Exercises[0].Text, Is.EqualTo("Push Ups"));
        Assert.That(result.Exercises[0].Value, Is.EqualTo(workout.WorkoutExercises.First().Id.ToString()));
        Assert.That(result.Exercises[1].Text, Is.EqualTo("Pull Ups"));
        Assert.That(result.Exercises[1].Value, Is.EqualTo(workout.WorkoutExercises.Last().Id.ToString()));

        Assert.That(result.Progressions, Is.Not.Null);
        Assert.That(result.Progressions.Count, Is.GreaterThan(0));
    }

    [Test]
    public void CreateAddWorkoutSetViewModel_WorkoutContainsNoExercises_DoesReturnViewModelWithEmptyExercisesButNotNullProgressions()
    {
        Workout workout = new Workout
        {
            Id = Guid.NewGuid(),
        ...
        Assert.That(result.Progressions, Is.Not.Null);
        Assert.That(result.Progressions.Count, Is.GreaterThanOrEqualTo(0));
    }
